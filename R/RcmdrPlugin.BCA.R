...
# Last modified on 10Nov11 by Dan Putler
...
scatterPlotBCA <- function(){
    ...
    checkBoxes(window=optionsParFrame, frame="optionsFrame", 
        ...
        initialValues=c(0, 0, 0, 0, 0, 1, 1, 1, 0), 
    ...
    onOK <- function(){
        ...
        if (.groups == FALSE) {
            doItAndPrint(paste("scatterplotBCA(", y, "~", x, log,
            ...
            doItAndPrint(paste("scatterplotBCA(", y, "~", x," | ", .groups,
                ", reg.line=", line, ", smooth=", smooth, ", spread=", spread, idtext,
                ", boxplots=", box, ", span=", span/100, jitter, xlab, ylab,
                cex, cex.axis, cex.lab, pch,
...
scatterPlotMatrixBCA <- function(){
    ...
    onOK <- function(){
        ...
        if (length(variables) < 3) {
            errorCondition(recall=scatterPlotMatrixBCA, message=gettextRcmdr("Fewer than 3 variable selected."))
            return()
            }
        line <- if("1" == tclvalue(lsLineVariable)) "lm" else "FALSE"
        smooth <- as.character("1" == tclvalue(smoothLineVariable))
        ...
        .activeDataSet <- ActiveDataSet()
        if (.groups == FALSE) {
           command <- paste("scatterplotMatrixBCA(~", paste(variables, collapse="+"),
           ...
           justDoIt(command)
            ...
            command <- paste("scatterplotMatrixBCA(~", paste(variables, collapse="+")," | ", .groups,
                ", reg.line=", line, ", smooth=", smooth, ", spread=", spread,
                ", span=", span/100, ", diagonal= '", diag,
                "', by.groups=", .linesByGroup,
                ", data=", .activeDataSet, subset, ")", sep="")
